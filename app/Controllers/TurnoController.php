...
class TurnoController extends BaseController
{
        ...
        foreach ($turnos as &$turno) {
            ...
            if ($session->get('rol') == 'Administrador') {
                ...
                if ($turno['estado'] == 'Atendiendo') {
                    $turno['estado'] = '<span class="badge badge-info">Atendiendo</span>';
                ...
                if ($turno['estado'] == 'Atendiendo') {
                    $turno['estado'] = '<span class="badge badge-success">Atendiendo</span>';
                    $turno['accion'] = '<button class="btn btn-success btn-sm finalizar" onclick="finalizarTurno(' . $turno['id'] . ')"><i class="fas fa-check"></i></button> ';
                    if ($turno['trabajador_id'] == null) {
                        $turno['accion'] .= '<button class="btn btn-primary btn-sm agregar-servicio" onclick="mostrarModalAgregarServicio(' . $turno['id'] . ')" disabled><i class="fas fa-plus"></i></button>';
                    } else {
    ...
    public function agregarServicioProducto()
    {
        ...
        if (!empty($servicioId)) {
            ...
            if ($servicio && $turno) {
                ...
                return $this->response->setJSON(['success' => false, 'message' => 'Servicio o turno no encontrado.']);
            }
        }
    ...
    // finalizar los turnos Atendiendos
    public function atenderTurno($id)
    {
        ...
        if (!empty($data['trabajador_id'])) {
            ...
            try {
                $this->turno->update($id, ['estado' => 'Atendiendo']);
                // $this->turnoServicio->where('turno_id', $data['id'])->set($data['trabajador_id'])->update();
                echo json_encode(['success' => true, 'message' => 'Turno marcado como Atendiendo.']);
            } catch (\Exception $e) {
                echo json_encode(['success' => false, 'message' => 'Error al marcar el turno como Atendiendo.']);
            }
        } else {
            echo json_encode(['success' => false, 'message' => 'No se puede marcar como Atendiendo, porque no ha asignado un empreado/a.']);
        }
    }
 ...
 try {
    ...
    /**
     * Finaliza un turno, registra el ingreso neto de la empresa en movimientos de caja.
     */
    public function finalizarTurno($id)
    {
        try {
            // Cambiar estado a Finalizado en la tabla agenda (turno)
            $this->turno->update($id, ['estado' => 'Finalizado']);

            $movimientoCajaModel = new \App\Models\MovimientoCajaModel();
            $turnoServiciosModel = new \App\Models\TurnoServiciosModel();
            $turnoProductosModel = new \App\Models\TurnoProductosModel();

            // 1. Obtener el turno principal (agenda)
            $turno = $this->turno->find($id);
            $ingresoEmpresa = 0;
            if ($turno && isset($turno['precio']) && isset($turno['pago_empleado'])) {
                $ingresoTurno = floatval($turno['precio']) - floatval($turno['pago_empleado']);
                if ($ingresoTurno > 0) {
                    $ingresoEmpresa += $ingresoTurno;
                }
            }

            // 2. Sumar servicios adicionales (tabla turno_servicios)
            $servicios = $turnoServiciosModel->where('turno_id', $id)->findAll();
            foreach ($servicios as $servicio) {
                $ingresoServicio = floatval($servicio['precio_servicio']) - floatval($servicio['pago_empleado']);
                if ($ingresoServicio > 0) {
                    $ingresoEmpresa += $ingresoServicio;
                }
            }

            // 3. Sumar productos (tabla turno_productos)
            $productos = $turnoProductosModel->where('turno_id', $id)->findAll();
            $productoModel = new \App\Models\ProductoModel();
            // Verificar stock antes de descontar
            foreach ($productos as $producto) {
                $productoEnBD = $productoModel->where('nombre', $producto['nombre_producto'])->first();
                if (!$productoEnBD || $productoEnBD['cantidad'] < $producto['cantidad'] || $productoEnBD['cantidad'] <= 0) {
                    echo json_encode(['success' => false, 'message' => 'El producto "' . $producto['nombre_producto'] . '" no tiene suficiente stock para finalizar el turno.']);
                    return;
                }
            }
            foreach ($productos as $producto) {
                $ingresoProducto = floatval($producto['subtotal']);
                if ($ingresoProducto > 0) {
                    $ingresoEmpresa += $ingresoProducto;
                }
                // // Descontar cantidad del inventario
                // $productoModel->decrement('cantidad', $producto['cantidad'], ['nombre' => $producto['nombre_producto']]);
            }

            // Registrar movimiento en caja solo si hay ingreso
            if ($ingresoEmpresa > 0) {
                $descripcion = "Ingreso por finalización de turno #$id (Servicios y productos)";
                $movimientoCajaModel->crearMovimiento('ingreso', $ingresoEmpresa, $descripcion);
            }

            echo json_encode(['success' => true, 'message' => 'Turno marcado como finalizado y movimiento registrado.']);
        } catch (\Exception $e) {
    ...
    public function removeExpiredTurnos()
    {
        ...
        $twentyMinutesAgo = date('Y-m-d H:i:s', strtotime('-30 minutes'));
    ...
    // listar Trunos Atendiendos y anulados
