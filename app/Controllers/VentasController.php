...
class VentasController extends BaseController
{
    ...
    public function realizar()
    {
        try {
            ...
            if (!is_array($input)) {
                return $this->response->setJSON(['success' => false, 'message' => 'Payload JSON inválido']);
            }
            // Obtener datos
            $session     = session();
            ...
            if (!$clienteId) {
                return $this->response->setJSON(['success' => false, 'message' => 'Cliente no seleccionado.']);
            }
            if (empty($productos)) {
            ...
            foreach ($productos as $producto) {
                $totalPagar += floatval($producto['valorTotal'] ?? 0);
            }

            // Verificar monto pagado
            if ($montoPagado < $totalPagar) {
                return $this->response->setJSON(['success' => false, 'message' => 'El monto pagado es menor al total de la venta.']);
            }
            ...
            foreach ($productos as $producto) {
                $productoEnBD = $this->productoModel->where('nombre', $producto['nombre'])->first();
                if (!$productoEnBD || $productoEnBD['cantidad'] < $producto['cantidad'] || $productoEnBD['cantidad'] <= 0) {
                    return $this->response->setJSON([
                        'success' => false,
                        'message' => 'El producto "' . $producto['nombre'] . '" no tiene suficiente stock para completar la venta.'
                    ]);
                }
            }
            ...
            $mov = new MovimientoCajaModel();
            $mov->crearMovimiento('ingreso', $totalPagar, 'Venta ID ' . $ventaId);

            return $this->response->setJSON(['success' => true, 'message' => 'Venta registrada correctamente.']);
        } catch (\Throwable $e) {
            // Registrar en log y devolver detalles para depuración
