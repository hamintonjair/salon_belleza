        ...
        <script>
                ...
                carrito.forEach((item, index) => {
                    tbody.append(`
                        <tr>
                            ...
                            <td>${item.cantidad}</td>
                            <td><input type="number" class="descuento form-control" data-index="${index}" value="${item.descuento > 0 ? (item.descuento / (item.precioUnitario * item.cantidad) * 100).toFixed(2) : ''}" onkeyup="actualizarDescuento(event)" /></td>
            ...
            function confirmarVenta() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            swal({ title: "Venta realizada", text: response.message || 'Venta confirmada con éxito.', icon: "success", button: "OK" })
                                .then(() => {
                                    carrito = [];
                                    mostrarCarrito();
                            ...
                            swal({ title: "Error", text: response.message || 'Error al confirmar la venta.', icon: "error", button: "OK" });
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Error al confirmar venta:', status, error, xhr.responseText);
                        swal({
                            title: "Error",
